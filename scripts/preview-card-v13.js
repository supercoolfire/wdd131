/**
 * preview-card.js v1.3 (Path-Corrected)
...
// 1. HOVER TRIGGER: Set the state on this specific card
card.addEventListener('mouseenter', () => {
  // Check if this specific card has already triggered the load
  if (card.dataset.previewLoaded !== 'true') {
    loadFullPreview(card, url);
    card.dataset.previewLoaded = 'true'; // Set boolean state ONLY for this card
  }
});

// 2. CLICK TRIGGER: Check the state of the specific clicked card
document.addEventListener('click', function(event) {
  const link = event.target.closest('a');
  if (!link || !link.querySelector('.preview')) return; 

  // Find the parent card belonging to this specific clicked link
  const currentCard = link.closest('.card'); // Replace '.card' with your actual card class/tag
  if (!currentCard) return;

  // Check the boolean state of THIS card only
  if (currentCard.dataset.previewLoaded !== 'true') {
    // If not loaded yet, block the click!
    event.preventDefault(); 
    
    // Trigger load only for this card
    loadFullPreview(currentCard, url);
    currentCard.dataset.previewLoaded = 'true'; 
    
    console.log("Click blocked for this card. Initiating preview load.");
  } else {
    // If it IS true, bypass preventDefault completely.
    // The browser will naturally follow the link on the second click.
    console.log("Preview already active for this card. Proceeding to link.");
  }
});          
